...
### 新增 Added（CN fork）
- **跳帧防闪烁（VrnrAntiFlicker，默认开）**：开启隔帧推理（VRNR）时的三项画质修复，
  可通过 ini / 游戏内面板 / 控制台 / 管理器调试页四端开关：
  - **P0 时间坡道**：连续跳帧时 edit 强度按 100% → 90% → 72% → 55% 缓降，
    消除运动边界「edit 瞬间消失/瞬间满强度回归」的 30Hz 方波突变；
  - **P1 权重空间平滑**：跳帧淡出权重改用 3×3 邻域平均亮度差计算，
    原生噪点不再逐帧驱动权重抖动（噪点呼吸感根源）；
  - **P2 高光守恒对称化**：跳帧同样执行 HDR 高光钳制（比例沿用上一推理帧），
    消除高光区两条链路的亮度差。
- 实现约束：零新增 GPU 资源、不碰 SRV/根签名/描述符布局；resolve 常量块
  10 → 12（新增 `vrnrRamp` / `vrnrAf`，cbuffer 末尾追加，两端同步）。
- 关闭开关时行为与 0.6.2 完全一致。
...
> **注**：0.7.0 – 0.7.3 的 VRNR 2.0 实验功能（可调推理间隔 / 跳帧混合 / 动态跳帧）在《剑星》+ ReShade 环境引发显存与 NGX 句柄问题，已整体回退至本版本状态。算法层与上游 `DLSSNR-Cost-Scaler` main 分支保持一致，本仓库只维护图形界面层。
