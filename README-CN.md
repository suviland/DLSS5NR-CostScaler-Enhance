...
| VRNR 跳帧 | 固定隔帧 | **可视化开关 + 跳帧防闪烁**（时间坡道 / 空间平滑 / 高光守恒对称化） |
...
开启隔帧推理（VRNR）后，推理帧与跳过帧交替显示容易产生亮度方波与噪点呼吸感。本分支的三重修复（总开关 `VrnrAntiFlicker`，默认开）：

- **时间坡道**：连续跳帧时残差强度按 100% → 90% → 72% → 55% 缓降、恢复推理自动回满，消除运动边界的突变；
- **空间平滑**：淡出权重改用 3×3 邻域平均亮度差计算，原生噪点不再逐帧驱动权重抖动；
- **高光守恒对称化**：跳帧同样执行 HDR 高光钳制，高光区两条链路亮度一致。

零新增 GPU 资源、不碰着色器资源绑定；关闭开关时行为与 0.6.2 完全一致。
...
- **隔帧推理（VRNR）值得开吗？** 开启后配合 **跳帧防闪烁**（`VrnrAntiFlicker`，默认开）体验远好于裸跳帧：噪点呼吸感与运动边界突变都被平滑。若在特定游戏中仍观感不佳，保持 `EnableAlternatingFrames = 0` 即可。
